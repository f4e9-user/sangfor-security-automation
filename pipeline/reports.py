...
def _evidence(row: dict[str, str]) -> dict[str, str]:
    return {
        ...
        "behavior_confidence_score": row.get("behavior_confidence_score", ""),
        "allowed_rate": row.get("allowed_rate", ""),
        "blocked_rate": row.get("blocked_rate", ""),
        "failed_rate": row.get("failed_rate", ""),
        "not_found_rate": row.get("not_found_rate", ""),
        "effective_response_rate": row.get("effective_response_rate", ""),
        "target_count": row.get("target_count", ""),
        "high_risk_intent": row.get("high_risk_intent", ""),
        "calibration_reasons": row.get("calibration_reasons", ""),
...
def _evidence_lines(row: dict[str, str], *, prefix: str = "") -> list[str]:
    ...
    return [
        ...
        f"  confidence: {row.get('behavior_confidence_score', '')}, allowed: {row.get('allowed_rate', '')}, blocked: {row.get('blocked_rate', '')}, failed: {row.get('failed_rate', '')}, 404: {row.get('not_found_rate', '')}",
        f"  targets: {row.get('target_count', '')}, high_risk_intent: {row.get('high_risk_intent', '')}, calibration: {row.get('calibration_reasons', '')}",
