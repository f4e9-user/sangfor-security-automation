...
class AutoBlocklistTests(unittest.TestCase):
    ...
    def test_block_posts_expected_headers_and_payload(self):
        ...
        self.assertEqual(
            call["url"],
            "https://192.0.2.116/api/batch/v1/namespaces/public/whiteblacklist?override=SKIPBACK",
        )
    ...
    def test_export_posts_expected_payload_and_returns_file(self):
        ...
        call = transport.calls[0]
        self.assertEqual(call["method"], "POST")
        self.assertEqual(call["url"], "https://192.0.2.116/api/v1/namespaces/public/export")
        self.assertEqual(call["headers"]["_cftoken"], "csrf-header")
        self.assertEqual(call["headers"]["Content-Type"], "application/json")
        self.assertEqual(
            json.loads(call["data"].decode("utf-8")),
    ...
    def test_download_export_uses_empty_post_body(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                call["url"],
                "https://192.0.2.116/php/loadfile.php?file=/export/blacklist_1783145132411.csv",
            )
