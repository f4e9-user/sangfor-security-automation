#!/usr/bin/env python3
"""Local-only Sangfor login helper that writes cookie/xid session JSON.
...
DEFAULT_BASE_URL = "https://192.0.2.118"
...
def infer_xid(page, cookies: list[dict]) -> str:
    for cookie in cookies:
        if cookie.get("name", "").lower() == "xid":
            return str(cookie.get("value", ""))
    value = page.evaluate(
        """
        () => window.localStorage.getItem('xid')
          || window.sessionStorage.getItem('xid')
          || window.localStorage.getItem('XID')
          || window.sessionStorage.getItem('XID')
          || ''
        """
    ...
    def capture(request):
        nonlocal request_xid
        header_value = request.headers.get("xid")
        if header_value:
            request_xid = header_value
    ...
    try:
        ...
        pass
    return request_xid or ""


def login(args: argparse.Namespace) -> dict:
    credentials = load_login_credentials(args.credentials_file, "sip") if args.credentials_file else None
    username = args.username or (credentials.username if credentials else "") or input("Sangfor username: ").strip()
    password = credentials.password if credentials else getpass.getpass("Sangfor password: ")
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            page.goto(f"{args.base_url.rstrip('/')}/ui/", wait_until="networkidle", timeout=60000)
            ...
            if "login" in page.url.lower():
                ...
                raise RuntimeError(f"login did not leave login page; current url={page.url}; body={body[:300]}")
            cookies = page.context.cookies()
            xid = infer_xid(page, cookies)
            if not xid:
                print("WARNING: xid was not found automatically; keepalive/export may need manual xid discovery.")
            session = {
